...
class ModelOptimizations:
    ...
    def performance(cls, model_name):
        ...
        """
        base_model_name = get_base_model_name(model_name)
        if False: # base_model_name == "Qwen2.5-7B":
            logger.info(
...
class ModelArgs:
    ...
    def __init__(
        ...
        if device is not None:  # Avoid issue with test_torch.py not having a device
            ...
            # )
            self.model_config["USE_FUSED_ALL_GATHER_MATMUL"] = False
            ...
            forced_prefill_grid = (12, 10) if self.is_p150_family else None
            prefill_rows = 10 if self.is_p150_family else 8  # TODO if BH = 10, if wh = 8
            ...
            # TODO: #26657 (if self.num_devices == 8 and os.getenv("ACTUAL_DEVICE", "") != "TG") should be refactored,
            # and investigate if ACTUAL_DEVICE environment variable is still used
            n_dim = (
            ...
            self.model_config["XQKV_PREFILL_PROGCFG"] = lambda seq_len: ttnn.MatmulMultiCoreReuseMultiCastProgramConfig(
                compute_with_storage_grid_size=forced_prefill_grid if self.is_p150_family else (8, 8),
    ...
    def _set_params_from_dict(self, config):
        ...
        if self.base_model_name == "Qwen2.5-7B" and self.num_devices not in [0, 1, 2, 4]:
            raise AssertionError(
                "Qwen2.5-7B is only supported on 1, 2, or 4 devices, run on an N150, N300, or use MESH_DEVICE=N150x4"
            )
    ...
    def find_grid(self, N):
        ...
        """
        max_rows = 10 if self.is_p150_family else 8
        max_cols = 12 if self.is_p150_family else 8
    ...
    def find_grid_k_n(self, K, N):
        ...
        """
        max_rows = 10 if self.is_p150_family else 8
        max_cols = 12 if self.is_p150_family else 8  # Maximum number of rows or columns
